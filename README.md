...
| The target theorem over the full fundamental domain | **NOT proven** — three-piece program, all pieces measured (PROOF_SKELETON.md §5): **bulk** coarse tiles validated 6/6 and chain-certified (~$25–50k CPU today, $5–15k post mechanical stack); **collar** = θ-scaled tiles under the measured law defect = c(β)·θ — demonstrated 48/48 on the face census incl. the β₃=±π/3 branch walls (signed analytic pair rates, ~6 s/tile, negligible cost); **face** = 2-param walk with pair-level obstruction, margin 0.09–0.17 — demonstrated 5/5 (~5 s/tile, negligible cost). Structural unknowns: nearly closed — the corner core decoded to three lines that all certify pointwise (NOTES §4.48–4.49); open residue = line-crossing points, the θ=0 corner anchor (MU continua), and a termination argument for the corner recursion. Remaining: that residue, the certified-grade pass, and the bulk campaign run |
